...
exports.handler = async (event) => {
    const { serial } = event.pathParameters;
  
    if (!serial) {
      return sendError(400, 'Missing serial number');
    }
  
    // Find the index of the product by serial number
    const index = products.findIndex((product) => product.serial === serial);
